...
テキストメモは即時切り替えのままに、画像メモが絡む切り替えにだけアニメーションを適用します。適用するアニメーションは[設定ページ](./settings-page.md)から選択できます。

## 使い方

[設定ページ](./settings-page.md)の「表示 > 画像の切り替えアニメーション」で選択します。選択肢は次のとおりです。
