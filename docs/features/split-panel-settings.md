...
フッター左から 2 番目の `sidebar` ボタンで、設定パネルの開閉をトグルします（幅 380pt、アニメーション付き。左端のボタンは[フォルダツリー](./folder-sidebar.md)のトグルです）。パネル開閉でウィンドウ幅が変わっても、ボタンはウィンドウ左下に固定されたままなので、同じ場所をクリックするだけで開閉できます。
...
パネルを開くとウィンドウ最小幅が `320 + 380` に広がり、閉じると `320` に戻ります（左ペインを開いている場合はさらにその幅ぶん広がります）。

なお[フォルダツリー](./folder-sidebar.md)の追加にあわせて、フッターはメモ列の中ではなくウィンドウ最下段いっぱいに敷く構成へ変更されました。左右どちらのペインを開いてもトグル類の位置は変わりません。

## 設定パネルでできること

[フォルダツリー](./folder-sidebar.md)の追加にあわせて、ファイル操作は左ペインへ移しました。設定パネルは「巡回の順番」と「巡回のしかた」に専念します。

| 区分 | 操作 |
| --- | --- |
| ローテーション順 | ドラッグ並べ替え、巡回対象の有効/無効トグル、更新日時の確認 |
| 動作設定 | ローテーション間隔、アイドル時間、[画像切り替えアニメーション](./image-transition.md) |

フォルダの選択、メモ／フォルダの作成、削除、Finder で開く、再スキャンは[左ペイン](./folder-sidebar.md)に集約されています。
